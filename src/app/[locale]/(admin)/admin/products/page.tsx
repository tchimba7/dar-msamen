...
export default async function AdminProductsPage({
  ...
  const [allCategories, myProducts] = await Promise.all([
    db
    ...
    <section className="space-y-6">
      <div className="flex items-center justify-between gap-3">
        <h1 className="text-3xl font-bold text-amber-950">
          {locale === "fr" ? "Mes produits" : "منتجاتي"}
        </h1>
      ...
      <Card>
        ...
        <form action={createAdminProductAction} className="mt-4 space-y-2">
      ...
      <Card>
        <CardTitle>{locale === "fr" ? "Gérer mes produits" : "إدارة منتجاتي"}</CardTitle>
        <div className="mt-4 space-y-3">
          {myProducts.length === 0 ? (
            <p className="text-sm text-amber-800">{locale === "fr" ? "Aucun produit créé." : "لا توجد منتجات منشأة."}</p>
          ) : (
            myProducts.map((product) => (
              <form key={product.id} action={updateAdminProductAction} className="space-y-2 rounded-md border border-amber-200 p-3">
                <input type="hidden" name="locale" value={locale} />
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    formAction={deleteAdminProductAction}
