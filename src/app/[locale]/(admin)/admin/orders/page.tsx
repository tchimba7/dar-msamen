...
export default async function AdminOrdersPage({
  ...
  const recentOrders = await (whereClause ? ordersQuery.where(whereClause) : ordersQuery)
    .orderBy(sortOrder)
    .limit(30);
  const pendingCount = recentOrders.filter((order) => order.status === "PENDING").length;
  const delayedCount = recentOrders.filter((order) => order.status === "DELIVERY_DELAYED").length;
    ...
    <section className="space-y-6">
      <div className="flex items-center justify-between gap-3">
        <h1 className="text-3xl font-bold text-amber-950">
          {locale === "fr" ? "Commandes clients" : "طلبات الزبائن"}
        </h1>
        <Link href={`/${locale}/admin`}>
          <Button variant="secondary">
            {locale === "fr" ? "Retour espace admin" : "العودة لفضاء الإدارة"}
          </Button>
        </Link>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card className="p-5">
          <CardTitle>{locale === "fr" ? "En attente" : "قيد الانتظار"}</CardTitle>
          <CardDescription>{pendingCount}</CardDescription>
        </Card>
        <Card className="p-5">
          <CardTitle>{locale === "fr" ? "Livraisons retardées" : "توصيل متأخر"}</CardTitle>
          <CardDescription>{delayedCount}</CardDescription>
        </Card>
        <Card className="p-5">
          <CardTitle>{locale === "fr" ? "WhatsApp à revoir" : "واتساب يحتاج مراجعة"}</CardTitle>
          <CardDescription>{failedWhatsAppCount}</CardDescription>
        </Card>
      </div>
      ...
      <Card>
        ...
        <form className="mb-4 grid gap-2 rounded-md border border-amber-200 bg-white p-3 md:grid-cols-5" method="get">
          ...
          <div className="flex gap-2">
            ...
            <Link href={`/${locale}/admin/orders`} className="rounded-md border border-amber-300 px-3 py-2 text-sm font-semibold text-amber-900">
